...
xvalue(2:end-1, 2:end-1, :) = im(:,:,:,1);
xvalue(1,2:end-1,:) = im(1,:,:,1);
xvalue(end,2:end-1,:) = im(end,:,:,1);
xvalue(:,1,:) = xvalue(:,2,:);
xvalue(:,end,:) = xvalue(:,end-1,:);

yvalue = zeros(Nx + 2, Ny + 2, Tx);
yvalue(2:end-1, 2:end-1, :) = im(:,:,:,2);
yvalue(1,2:end-1,:) = im(1,:,:,2);
yvalue(end,2:end-1,:) = im(end,:,:,2);
yvalue(:,1,:) = yvalue(:,2,:);
